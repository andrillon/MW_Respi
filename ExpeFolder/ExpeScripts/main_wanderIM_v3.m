...
if flag_skipbaseline==0 && flag_escp==0
    ...
    DrawFormattedText(w, 'Nous allons maintenant enregistrer comment votre cerveau réagit aux stimuli clignotants\n\n Appuyez sur une touche quand vous êtes prêt', 'center', 'center', [255 255 255]);
    Screen('Flip',w);
    KbWait(-1);
    KbReleaseWait(-1);
...
if flag_escp==1
    ...
    if flag_skiptraining==0
        ...
        DrawFormattedText(w,sprintf('Votre performance était de\n%2.1f %% (appuis)\n%2.1f %% (non-appuis)\n%1.2fs (temps de réaction)\n\nAppuyez sur une touche quand vous êtes prêt',perfGO,perfNOGO,meanRT), 'center', 'center', [255 255 255]);
        Screen('Flip',w);
        KbWait(-1);
        KbReleaseWait(-1);
        Screen('Flip',w);
...
while nblock < maxblock && flag_escp==0
    ...
    DrawFormattedText(w, sprintf('Partie %g sur %g\n\nAppuyez sur une touche quand vous êtes prêt',nblock,length(block_perm)), 'center', 'center', [255 255 255]);
    Screen('Flip',w);
    KbWait(-1);
    KbReleaseWait(-1);
    Screen('Flip',w);
...
DrawFormattedText(w, 'Félicitations!\n\nVous avez terminé!\n\nMerci de votre participation', 'center', 'center', [255 255 255]);
Screen('Flip',w);
WaitSecs(5);
end
